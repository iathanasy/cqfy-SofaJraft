...
public class RocksDBLogStorage implements LogStorage, Describer {
     ...
     * @date:2023/12/20
     * @方法描述：该方法的作用就是在生成快照之后，清除数据库中firstIndexKept之前的旧日志
     */
    @Override
     ...
     * @author：陈清风扬，个人微信号：chenqingfengyangjj。
     * @date:2023/12/20
     * @方法描述：该方法的作用很明确，就是清除startIndex——firstIndexKept索引之间的日志，包括startIndex本身
     */
    private void truncatePrefixInBackground(final long startIndex, final long firstIndexKept) {
        //又是在异步线程中执行清除日志的操作，如果不使用异步线程，使用的就是disruptor的批处理器的县城了，显然会阻塞disruptor
        ThreadPoolsFactory.runInThread(this.groupId, () -> {
            //记录操作开始事件
            long startMs = Utils.monotonicMs();
            //上读锁，操作期间，数据库中的数据不能再被修改了
            this.readLock.lock();
            try {
                //得到数据库
                RocksDB db = this.db;
    ...
    public boolean appendEntry(final LogEntry entry) {
        if (entry.getType() == EnumOutter.EntryType.ENTRY_TYPE_CONFIGURATION) {
            ...
            try {
                ...
                final byte[] newValueBytes = valueBytes;
