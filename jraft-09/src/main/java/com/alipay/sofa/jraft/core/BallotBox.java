...
public class BallotBox implements Lifecycle<BallotBoxOptions>, Describer {
    ...
    public boolean commitAt(final long firstLogIndex, final long lastLogIndex, final PeerId peer) {
        ...
        try {
            ...
            //这里就是要给兜底判断，如果发现要处理的这批日志的最后一条索引比pendingMetaQueuependingMetaQueue队列的数据还多
            ...
            //接下来就是pendingIndex大于firstLogIndex的情况了，这个也很好理解，既然日志传输是Pipeline模式，并且每个复制器对象收到响应
            //也并不是同步的，很可能出现这样一种情况，整个集群中有5个节点，1号节点是领导者，领导者向四个跟随者发送了一批日志，索引为1-10
            //这批日志已经收到了集群过半节点的投票响应，比如2，3，4节点都回复响应了，领导者成功提交了第一批日志中索引为1-7的日志，pendingIndex也已经更新了
            //但这时候领导者的pendingIndex已经更新了，肯定比这时候，收到响应的要处理的这批日志的第一条索引大，所以要从pendingIndex这个已经提交的位置继续开始处理
            //否则就会重复提交日志，造成状态机重复执行命令了，但这种情况应该也不会发生，因为一批日志在跟随者节点被处理，如果要成功，就全都处理成功，不会出现一半成功一半失败的情况
            //当然这里也有可能就是新的领导者把旧日志同步给某个进度落后的节点时，可能会出现的情况。比如领导者现在有1-4索引的日志，这时候有一个节点故障了，在故障期间，领导者先接收到了4条日志
            //把这四条日志同步给其他跟随者节点，然后领导者成功提交了日志，这时候pendingIndex就变成了9，然后故障的节点恢复正常了，这时候领导者又接收到了两条日志，现在一共有10条日志了
