...
public class MapStateMachine  extends StateMachineAdapter{
    ...
    public boolean onSnapshotLoad(final SnapshotReader reader)  {
        //这里我解释一下，朋友们，我这个加载快照的方法显然很简陋，而且也不太严谨
        //我这里是个键值对，结果只加载的value，key就是“日志”字符串，是我自己写的，好像跟随者节点事先就知道似的
        //这是因为我在生成快照的时候就只生成了value，大家自己弄得时候可以严谨一些，看看怎么把键值对写入快照文件中
        //加载快照文件的时候还能还原每一条键值对，然后把键值对写入到跟随者的map中
        final LogSnapshotFile snapshot = new LogSnapshotFile(reader.getPath() + File.separator + "data");
        try {
            String s = snapshot.load();
            map.put("日志",s);
