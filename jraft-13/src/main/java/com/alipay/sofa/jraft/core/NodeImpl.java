...
public class NodeImpl implements Node, RaftServerService {
    ...
    //配置类上下文，这个类的对象就是配置变更操作对象
    //配置变更的过程以及各种状态都由该对象封装
    private static class ConfigurationCtx {
        //这个枚举类中的几个枚举对象代表的是配置阶段
        enum Stage {
            //配置没有变更的阶段，当集群初次启动的时候，配置变更状态就是STAGE_NONE
            //并且当集群并没有进行配置变更活动时，配置变更的状态也会被重置为STAGE_NONE
            STAGE_NONE,
        ...
        //配置变更进行到哪个阶段了
        Stage stage;
        //配置变更的数量，就是有多少节点发生了变动
        int nchanges;
        //配置变更版本号
        long version;
        //集群中最新的配置信息，其实就是添加了新的节点之后，所有地节点的集合
        List<PeerId> newPeers = new ArrayList<>();
        //集群中旧配置信息，其实就是旧的节点集合
        List<PeerId> oldPeers = new ArrayList<>();
        //要添加到集群中的新的节点的集合
        List<PeerId> addingPeers = new ArrayList<>();
        //下面都是和学习者相关的成员变量了，在我们的课程中并没有涉及到，所以就省略了
        //当然，有一点我必须跟大家说明，我把学习者略去了，并不是就不讲了，我可以负责任的和大家说
        //学习者变更和正式参与集群活动的节点的变更的流程都是一样的，都属于配置变更，大家掌握了正式参与集群活动的节点
        //的配置变更，也就掌握了学习者的变更流程，这一点大家要理解
        List<PeerId> newLearners = new ArrayList<>();
        List<PeerId> oldLearners = new ArrayList<>();
        //这个回调对象中定义着配置变更操作完成之后要立刻执行的回调方法
        Closure done;
        ...
        public ConfigurationCtx(final NodeImpl node) {
            super();
            this.node = node;
            //默认配置变更状态为STAGE_NONE，也就是没有进行变更的阶段
        ...
        void start(final Configuration oldConf, final Configuration newConf, final Closure done) {
            //再次检查当前集群的配置变更是否处于忙碌状态，也就是不等于Stage.STAGE_NONE，因为在集群没有进行配置变更的时候
            //配置变更状态就为Stage.STAGE_NONE
            if (isBusy()) {
                //如果集群当前的配置变更为忙碌状态，那就抛出异常，回复客户端集群繁忙响应
                if (done != null) {
            ...
            //判断当前配置变更对象中的回调对象是否为null
            if (this.done != null) {
                //如果不为null，说明已经在进行配置变更了，也要抛出异常，回复客户端错误响应
                if (done != null) {
        ...
        private void addNewPeers(final Configuration adding) {
            ...
            //遍历addingPeers集合，把集合中的每一个节点添加到集群中
            //注意，在我为大家提供的当前版本代码中，只有一个addPeer方法用来修改集群中的配置
            //所以这里addingPeers集合中只可能有一个要添加的节点
            //但这里既然用到了集合，就意味着在源码中肯定还有一种情况，那就是可以将一批节点添加到集群中
            //源码中的changePeers方法就是这个作用，大家可以去看一看，当然不管是一次添加几个节点，流程都是一样的
            //程序都会执行到这里
            for (final PeerId newPeer : this.addingPeers) {
                //为新添加的节点创建复制器对象，并把复制器对象交给复制器组对象来管理
                //注意，这里一旦为新添加的节点创建了复制器对象，领导者就会向新添加的节点发送探针请求
                //然后根据探针请求的响应决定是传输日志还是安装快照，总而言之，只要为新添加的节点创建了复制器对象
                //这个新添加的节点就要追赶领导者的进度了
                if (!this.node.replicatorGroup.addReplicator(newPeer)) {
                    ...
                    //在这里执行的onCaughtUp方法，主要是为了重置当前的ConfigurationCtx对象的状态和成员变量的值
                    onCaughtUp(this.version, newPeer, false);
                    //走到这里意味着为新节点创建复制器对象失败，这里就直接退出该方法
                    return;
                }
                //创建一个OnCaughtUp对象，这个对象中定义的方法会在新添加的节点追上领导者的进度之后被回调
                final OnCaughtUp caughtUp = new OnCaughtUp(this.node, this.node.currTerm, newPeer, this.version);
                //在这里定义了一个超时时间，这里我要解释一下，一个新的节点添加到集群中，肯定要追赶领导者的进度，但是追赶进度也不是没有时间限制的
                //jraft框架会为追赶领导者进度的操作定义一个时间限制，不能无限制追赶，否则新添加的节点都掉线了，还在追赶领导者进度，这怎么可能啊？
                //因此定义一个超时时间，超过这个时间还没有追赶上，就认为追赶操作出现问题了
                final long dueTime = Utils.nowMs() + this.node.options.getElectionTimeoutMs();
                //在这里调用了replicatorGroup.waitCaughtUp方法，就是在等待新添加的节点追赶上领导者的进度
                //当然，在replicatorGroup.waitCaughtUp方法内部，实际上还是调用了Replicator复制器对象的waitForCaughtUp方法来执行这个操作的
                if (!this.node.replicatorGroup.waitCaughtUp(this.node.getGroupId(), newPeer,
                    ...
                    //操作失败则重置当前的ConfigurationCtx对象的状态和成员变量的值
                    onCaughtUp(this.version, newPeer, false);
                    return;
                }
            }
        ...
        //这个方法会在新添加的节点追赶进度失败或者成功后被调用，调用的目的很明确，就是重置当前的ConfigurationCtx对象的状态和成员变量的值
        void onCaughtUp(final long version, final PeerId peer, final boolean success) {
            ...
            //判断当前集群配置变更的状态是否作为正在追赶状态，大家别忘了，在上面的start方法中，当前集群正要进行配置变更操作
            //然后将当前节点配置变更的状态设置为Stage.STAGE_CATCHING_UP了，紧接着新添加到集群中的节点就要开始追赶领导者进度
            //不管进度追赶成功或者失败，总之追赶进度的操作执行完毕之后，就要调用当前的onCaughtUp方法，所以，这个时候，集群目前的配置变更状态
            //仍然为 Stage.STAGE_CATCHING_UP，当然，在该方法法中，接下来就要根据操作执行的结果来重置集群目前的配置变更状态了
            Requires.requireTrue(this.stage == Stage.STAGE_CATCHING_UP, "Stage is not in STAGE_CATCHING_UP");
            //在这里判断追赶领导者进度的操作是否成功
            if (success) {
                //如果成功，就重置addingPeers集合
            ...
            //重置当前的ConfigurationCtx对象的状态和成员变量的值
        ...
        //该方法的作用就是用来重置当前的ConfigurationCtx对象的状态和成员变量的值
        void reset(final Status st) {
            //判断执行结果的状态是否为成功状态
            if (st != null && st.isOk()) {
                //走到这里就意味着是成功状态，这时候就可以把就配置中的节点的复制器全都停止了
                //注意，这里大家仍然要多思考一下，旧的配置难道不会和新的配置有重叠吗？
                //假如说集群中有3个节点，我向集群中新添加了1个节点，然后又移除了一个旧的节点
                //新节点追赶领导者进度也成功了，这个时候要重新把集群当前的配置变更状态更新为Stage.STAGE_NONE
                //因为不再进行配置变更操作了，与此同时，我是不是还要把移除的那个节点对应的复制器对象也终止了？
                //这时候就有一个很重要的操作，那就是对比新配置和就配置的不同点，肯定就能找到那个被移除的节点
                //然后直接终于被移除的节点的复制器对象即可，下面这个方法执行的就是这样的操作
                this.node.stopReplicator(this.newPeers, this.oldPeers);
                this.node.stopReplicator(this.newLearners, this.oldLearners);
            } else {
                //走到这里意味着追赶领导者进度的操作没有成功
                //这时候就要把之前为每一个新添加进来的节点创建的复制器对象都给终止了
                this.node.stopReplicator(this.oldPeers, this.newPeers);
                this.node.stopReplicator(this.oldLearners, this.newLearners);
            }
            //重置集合，其实就是把三个集合成员变量清空了
            clearPeers();
            //更新版本号，这就意味着已经发生过一次配置变更，配置信息版本更新了
            this.version++;
            //将集群配置变更节点设置为Stage.STAGE_NONE，配置变更操作结束了，当然要把状态重置为初始值
            this.stage = Stage.STAGE_NONE;
            //变更的节点数量也重置为初始值
            this.nchanges = 0;
            //下面就到了执行回调方法的时候了，注意，这里的回调方法就是用来通知客户端执行结果的
            if (this.done != null) {
                //不管执行追赶领导者进度的操作是否成功，最终都要执行回调方法，通知客户端配置变更操作结果
        ...
        void flush(final Configuration conf, final Configuration oldConf) {
            ...
            if (oldConf == null || oldConf.isEmpty()) {
                ...
                //如果旧配置不为空，说明集群以前就有配置信息，这时候要更新为新的配置信息
                //就会把集群配置变更状态设置为新旧配置过度阶段，代表正在进行配置变更
                this.stage = Stage.STAGE_JOINT;
                this.oldPeers = oldConf.listPeers();
                this.oldLearners = oldConf.listLearners();
            }
            //将新配置立刻应用到集群中的操作就在下面这个方法中执行
        ...
        //执行配置变更的下一操作的方法
        void nextStage() {
            //判断目前集群配置变更阶段是否处于繁忙状态，现在肯定应该处于繁忙状态，因为之前已经把集群配置变更阶段更新为Stage.STAGE_CATCHING_UP了
            Requires.requireTrue(isBusy(), "Not in busy stage");
            //得到当前集群配置变更的状态
            switch (this.stage) {
                case STAGE_CATCHING_UP:
                    //走到这里意味着集群当前的配置变更阶段为Stage.STAGE_CATCHING_UP
                    if (this.nchanges > 0) {
                        //并且集群节点配置确实发生了变化，这说明新添加的节点已经追赶领导者进度成功了
                        //接下来就可以把集群配置变更状态更新为Stage.STAGE_JOINT过渡阶段
                        //然后将新配置立刻应用到整个集群上即可
                        this.stage = Stage.STAGE_JOINT;
                        //把新的配置立刻应用到集群中
                ...
                case STAGE_JOINT:
                    //走到这里意味着集群配置变更阶段为Stage.STAGE_JOINT，并且已经成功把新配置应用到集群中了
                    //这时候就可以把集群配置变更阶段更新为Stage.STAGE_STABLE，代表集群配置变更结束，已经稳定了
                    this.stage = Stage.STAGE_STABLE;
                    // 在节点上应用新的配置，只包括新的节点和学习者，旧节点被移除
                    this.node.unsafeApplyConfiguration(new Configuration(this.newPeers, this.newLearners), null, false);
                    break;
                case STAGE_STABLE:
                    //走到这里意味着集群配置变更状态为STAGE_STABLE，说明集群配置变更操作已经执行完毕了
                    //然后判断一下领导者是否在集群的最新配置中，如果不在就意味着领导者要下台了
                    final boolean shouldStepDown = !this.newPeers.contains(this.node.serverId);
                    //在这里重置当前的ConfigurationCtx对象的状态和成员变量的值
                    reset(new Status());
                    //如果领导者应该下台，就执行身份降级操作
                    if (shouldStepDown) {
                        this.node.stepDown(this.node.currTerm, true, new Status(RaftError.ELEADERREMOVED, "This node was removed."));
                    }
                    break;
                case STAGE_NONE:
                    //正常来说，按照集群配置变更流程，程序是不会执行到这里的，因为STAGE_NONE是集群配置变更阶段的最初状态，一旦进行配置变更操作
                    //状态就发生改变了，不会进入到这个分支
                    //所以这里只是校验一下即可
    ...
    private void unsafeApplyConfiguration(final Configuration newConf, final Configuration oldConf,
        ...
        //接下来的流程就很简单了，要想把最新的配置应用到集群中，其实就是把配置变更日志传输给集群中的每一个节点就行，也就是让配置变更日志条目在集群中达成共识
        //所以接下来就要创建一个配置变更的日志条目
        ...
        //创建一个回调对象，该对象中的方法会在配置变更的日志条目在集群中达成共识后被回调
    ...
    private class ConfigurationChangeDone implements Closure {
        ...
        //这个方法会在配置变更日志在集群中达成共识之后被回调
        @Override
        public void run(final Status status) {
            if (status.isOk()) {
                //在下面这个方法中会执行nextStage方法，程序就会执行到nextStage方法的case STAGE_STABLE分支
                //这个时候配置变更已经完毕了，所以要重置当前的ConfigurationCtx对象的状态和成员变量的值
    ...
    public void onConfigurationChangeDone(final long term) {
        ...
        try {
            ...
            //执行到这里，就意味着最最新配置已经在集群的各个节点之间达成共识了，也就意味着配置变更的操作已经结束了
            //接下来就可以在nextStage方法中重置当前的ConfigurationCtx对象的状态和成员变量的值
    ...
    public void readIndex(ReadOnlyOption readOnlyOptions, byte[] requestContext, ReadIndexClosure done) {
        //这段代码暂且注释掉，这是优雅停机的逻辑，反正马上就要优雅停机了，所以代码就不删除了
...
//        }
    ...
    public void handleReadIndexRequest(final RpcRequests.ReadIndexRequest request, final RpcResponseClosure<RpcRequests.ReadIndexResponse> done) {
        ...
        try {
            ...
            switch (this.state) {
                case STATE_LEADER:
                    //走到这里意味着是领导者，那么直接处理读请求即可，领导者应用的数据就是最新的，可以直接处理读请求
                    readLeader(request, RpcRequests.ReadIndexResponse.newBuilder(), done);
                    break;
                case STATE_FOLLOWER:
                    //走到这里就意味着当前节点是跟随者节点，那就要给领导者发送一个ReadIndexRequest请求，让领导者处理读操作
    ...
    //这个方法一会就会用到，就是根据配置信息，看看当前节点收到的票数是否超过了集群一半的节点
    ...
    //于是就会向领导者发送一个ReadIndexRequest请求，让领导者来处理读请求
     ...
     * @author：陈清风扬，个人微信号：chenqingfengyangjj。
     * @date:2024/3/15
     * @方法描述：领导者处理读请求的方法，如果该方法被调用了，说明当前节点就是领导者，并且客户端恰好就把读请求发送给领导者了，因为领导者中的数据都是最新的，状态机上应用的日志也都是最新的
     * 所以领导者可以直接处理这个读请求
     */
    private void readLeader(final RpcRequests.ReadIndexRequest request, final RpcRequests.ReadIndexResponse.Builder respBuilder,
        ...
        if (quorum <= 1) {
            ...
            //注意，这里成功返回成功响应，实际上就是直接返回给业务层了，业务层就可以直接读取数据返回给客户端了
            respBuilder.setSuccess(true)
                    //这里的操作最重要，把节点本身应用的最新日志索引返回
                    .setIndex(this.ballotBox.getLastCommittedIndex());
            closure.setResponse(respBuilder.build());
            //执行回调方法，通知业务层获取数据，上面设置的最新日志的索引会在执行下面这个回调方法中用到
            closure.run(Status.OK());
            return;
        }
        //走到这里意味着集群中不是一个节点，有很多节点，现在就获取领导者应用到状态集中最新的日志索引
        final long lastCommittedIndex = this.ballotBox.getLastCommittedIndex();
        //这里判断一下领导者最新应用的日志索引是否等于当前领导者的任期，如果不相等就意味着被应用的日志并不是在领导者所属任期内被应用的
        //可能是之前应用的，也就是说当前节点成为领导者之后还没有应用日志，这样的话就只能拒绝这个读请求
        //这里也许大家会有些疑惑，为什么非要领导者在自己任期内应用日志才行，请大家回忆一下raft共识算法，当一个节点当选为领导者之后
        //一般都要立即复制一条空日志给集群中各个节点，然后对这条空日志达成共识，这是因为只有这样，这条空日志之前的所有日志就都可以应用了
        //领导者也才能知道目前最新的数据究竟是哪些，如果不复制空日志，可能上一个任期中有些日志本来可以提交的，但一直没有提交也没有应用
        //这时候领导者直接处理读请求，有可能返回旧数据
        ...
        //走到这里就意味着当前节点为领导者，领导者中的数据也是最新的
        //直接把领导者应用的最新日志索引设置到响应中
        respBuilder.setIndex(lastCommittedIndex);
        if (request.getPeerId() != null) {
            //解析发送请求的节点信息，也就是解析跟随者节点的信息
            final PeerId peer = new PeerId();
            peer.parse(request.getServerId());
            //判断跟随着节点信息是否在集群中，如果不在直接返回错误响应
        ...
        //这里判断是否为租约模式，并且判断领导者是否在祖约有效时间内
        //if (readOnlyOpt == ReadOnlyOption.ReadOnlyLeaseBased && !isLeaderLeaseValid()) {
            //如果领导者租约超时了，就把租约模式改为只读模式，仍然使用只读模式处理读请求
        //    readOnlyOpt = ReadOnlyOption.ReadOnlySafe;
        //}
        //根据处理读请求的方式处理读请求
        switch (readOnlyOpt) {
            case ReadOnlySafe:
                //这个模式的处理策略非常简单，因为本身已经在领导者处理读请求了，领导者应用状态机的日志索引也是最新的
                //所以现在只要再次确保当前节点是领导者节点即可，这样就保证万无一失了
                ...
                //根据集群配置向集群中每一个跟随者节点发送心跳消息
                for (final PeerId peer : peers) {
                    if (peer.equals(this.serverId)) {
                        continue;
                    }//在这里发送心跳消息，当接收到心跳消息的响应后，会调用刚才创建的heartbeatDone对象中的run方法
                    //run方法中的逻辑也很简答，就是判断接收到的成功的心跳响应的数量是否超过了集群半数节点，超过了意味着当前节点就是领导者
                    this.replicatorGroup.sendHeartbeat(peer, heartbeatDone);
                }
                break;
            case ReadOnlyLeaseBased:
                //这里就是领导者租约模式，在租约模式下不会发生领导者选举，所以就可以直接返回成功响应即可
                respBuilder.setSuccess(true);
                closure.setResponse(respBuilder.build());
                //在这里执行回调方法，这个回调方法会把成功响应通知业务层，业务层会执行读取数据的操作
                //然后就可以返回客户端了
                closure.run(Status.OK());
                break;
        }
    ...
    //上面那个心跳消息的回调方法，接收到心跳响应后就会回调当前的方法
    private class ReadIndexHeartbeatResponseClosure extends RpcResponseClosureAdapter<RpcRequests.AppendEntriesResponse> {

        //只读请求响应构建器
        final RpcRequests.ReadIndexResponse.Builder respBuilder;
        //只读响应的回调对象，该对象中的run方法会在接收到只读响应后被回调
        final RpcResponseClosure<RpcRequests.ReadIndexResponse> closure;
        //集群中过半节点的数量
        final int quorum;
        //失败响应的阈值，当回复失败响应的数量达到下面这个成员变量的值了，就意味着确认领导者身份失败了
        ...
        public synchronized void run(final Status status) {
            if (this.isDone) {
                //这里就是判断本次确认领导者的操作结束了，直接退出方法即可
                //因为只要收到超过节点半数的成功响应就可以了，后续再收到响应都可以忽略了
                return;
            }
            //接收到的心跳响应是成功的
            if (status.isOk() && getResponse().getSuccess()) {
                ...
                //走到这里意味着响应是失败的，给失败次数加1
                this.ackFailures++;
            }
            //这里让ackSuccess+1是因为领导者自己还有一票呢，如果这个票数大于集群中过半节点的数量了
            //就意味着操作成功，返回一个成功响应即可
            //大家一定要理清楚这个成功响应时返回给谁的，大家要多看几遍，弄清楚
            //这个成功响应实际上还是会在ReadOnlyServiceImpl类中的ReadIndexResponseClosure回调对象中被处理
    ...
    //得到集群中所有节点的方法，注意，这里得到的是正式参与集群运转的节点
    @Override
    public List<PeerId> listPeers() {
        this.readLock.lock();
        try {//判断当前节点是否为领导者，如果不是领导者，就抛出异常
            if (this.state != State.STATE_LEADER) {
    ...
    public void addPeer(final PeerId peer, final Closure done) {
        ...
        this.writeLock.lock();
        try {//在这里校验一下，当前要添加的节点不应该包含在集群当前的配置信息中，否则添加的就不是新节点了
    ...
    private void unsafeRegisterConfChange(final Configuration oldConf, final Configuration newConf, final Closure done) {
        ...
        //走到这里就意味着当前节点是领导者，现在要进行配置变更操作了，所以要先检查一下，当前节点是否正在进行配置变更操作
        //如果正在进行配置变更操作，就退出当前方法，记录告警信息
        //这里我想多说几句，也许大家并没有意识到这一点，就是当集群第一次启动的时候，肯定会进行一次配置变更操作
        //因为集群刚启动，集群的配置信息肯定要被应用，所以会进行一次配置变更操作，这也就意味着配置变更状态也会发生改变
        //当一个节点成为领导者之后，会在becomeLeader方法中执行flush方法，flush方法就是刷新配置的操作，在这个流程中会对领导者配置变更的状态做一系列操作
        //我建议大家可以顺着flush方法的流程看一看，看看集群刚启动的时候，配置变更的操作是怎么执行的，要经过什么流程，当然，这些内容我会在文章中
        //详细地为大家讲解分析
        if (this.confCtx.isBusy()) {
        ...
        //在这里判断一下新的配置是否和旧的配置内容相同，如果内容相同则退出当前方法
        if (this.conf.getConf().equals(newConf)) {
            //因为新旧配置相同，所以就直接回复一个成功响应即可
    ...
    //这个类的对象也是一个回调对象，当新添加到集群中的节点追赶上了领导者的日志进度之后
    //这个对象中的run方法就会被回调
    private static class OnCaughtUp extends CatchUpClosure {
        ...
        @Override
        public void run(final Status status) {
            //在run方法中又调用了onCaughtUp方法
            this.node.onCaughtUp(this.peer, this.term, this.version, status);
        }
    }

    //当新添加到集群中的节点追赶上了领导者的日志进度之后，该方法就会被回调
    //这个方法是真正处理追赶完成之后流程的方法
    private void onCaughtUp(final PeerId peer, final long term, final long version, final Status st) {
        ...
        try {
            ...
            if (st.isOk()) {
                //走到这里意味着追赶进度成功了，调用onCaughtUp方法进入配置变更的下一阶段即可
                this.confCtx.onCaughtUp(version, peer, true);
                return;
            }
            //走到这里意味着追赶失败了，其实就是超过了限制时间，追赶操作超时了
            //但是新添加的节点仍然存活，并没有掉线
            ...
            //走到这里就是追赶操作失败了，并不是超时，这时候也要回调onCaughtUp方法
    ...
    public void shutdown(Closure done) {
        ...
        try {
            ...
            if (this.state.compareTo(State.STATE_SHUTTING) < 0) {
                ...
                //我认为这里是考虑到当前节点是领导者或者候选者，因为是领导者的话，当前节点需要销毁所有的复制器对象，清空投票箱等等
                ...
                }//在这里更新节点状态为State.STATE_SHUTTING
            ...
            //这里节点的状态应该还是为State.STATE_SHUTTING，所以会进入下面的分支
            if (this.state != State.STATE_SHUTDOWN) {
                if (done != null) {
                    //这里就是把调用当前方法时传入的回调对象添加到shutdownContinuations集合中
            ...
            ThreadPoolsFactory.runInThread(this.groupId, () -> {
                try {
                    ...
                    //                return;
                    //            }
                    //这个if分支，那么done就会被置为null了，就算赋值给了shutdownHook，这个shutdownHook也是null，所以这里会判断出shutdownHook为null
                    //就不会再执行done中的回调方法了
                    //至于回调方法被执行的时机，就放在了下面的afterShutdown方法中，当afterShutdown方法被执行的时候，说明所有组件都已经关闭了
                    //这时候执行最终的回调方法即可，然后把节点当前的状态更新为State.STATE_SHUTDOWN即可
    ...
    //这个方法会在程序中的所有组件都关闭了之后被调用，在这个方法中，会执行shutdownContinuations集合中的回调方法
    //也会把节点当前的状态更新为State.STATE_SHUTDOWN已经停止工作状态
    ...
    public List<PeerId> listAlivePeers() {
        this.readLock.lock();
        try {//判断调用当前方法的节点是不是领导者
            //只有领导者才能主动调用该方法，得到集群中存活的节点
            if (this.state != State.STATE_LEADER) {
                throw new IllegalStateException("Not leader");
            }//这个就是真正得到存活的结点的方法
            //Utils.monotonicMs()得到的是当前时间毫秒值
    ...
    private List<PeerId> getAliveNodes(final Collection<PeerId> peers, final long monotonicNowMs) {
        ...
        for (final PeerId peer : peers) {
            if (peer.equals(this.serverId)) {
                ...
                continue;
            }//当前时间减去领导者最后一次和遍历到的节点发送信息的时间
            //如果这个时间没有超过方法参数中的monotonicNowMs，就意味着节点还存活
     ...
     * @author：陈清风扬，个人微信号：chenqingfengyangjj。
     * @date:2024/7/7
     * @方法描述：我跟大家解释一下为什么不实现这个方法，这个方法就是转移领导者权力给方法参数中的peer节点
     * 也就是转移领导者的方法，但是这个方法我并没有给大家实现，因为这个方法的调用链和逻辑比较复杂，要是讲解的话至少得用一个章节的篇幅来讲解
     * 简单来说，这个功能的大概逻辑就是先对当前节点也就是领导者节点做一系列判断，如果判断通过，再让当前领导者下台，然后通知指定的peer节点
     * 让peer节点主动执行领导者选举操作。如果大家感兴趣，可以自己去看看源码，调用链非常清楚，按照我分析得简单逻辑，应该挺容易就看明白了
     */
    @Override
