...
import static com.alipay.sofa.jraft.option.ApplyTaskMode.Blocking;
import static com.alipay.sofa.jraft.option.ApplyTaskMode.NonBlocking;
    ...
    public Message handleAppendEntriesRequest(final RpcRequests.AppendEntriesRequest request, final RpcRequestClosure done) {
        ...
        try {//判断当前节点是否活跃
            ...
            if (localPrevLogTerm != prevLogTerm) {
                ...
                //这里返回的信息会被领导者处理，但是领导者具体处理的逻辑我还没有实现，在第四版本代码才会实现这个逻辑
                return RpcRequests.AppendEntriesResponse.newBuilder()
                        .setSuccess(false)
                        .setTerm(this.currTerm)
        ...
        try {//这个tasks集合中存放着32个LogEntryAndClosure对象
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (task.expectedTerm != -1 && task.expectedTerm != this.currTerm) {
                    ...
                    continue;
                } //下面这段代码是为每一个日志条目对象创建了投票箱
                //这里这段代码我就先注释了，不删除了，因为第四版本就要用到，大家看的时候直接跳过即可
